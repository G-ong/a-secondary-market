    ...
    <div class="right">
      ...
      <!-- 未完成 -->
          <div v-for="noneSell in noneSells" :key="noneSell.id">
            <div v-if="noneSell.status=='0'">
            <div class="white">
            <p class="state">未完成</p>
            <p class="name">{{noneSell.goods.goodsName}}</p>
            <p class="time">{{noneSell.createTime}}</p>
            <button @click="delete2(noneSell.goods.id)" class="btn1">删除</button>
            <button @click="change1(noneSell.goods.id)" class="btn2">修改</button>
              </div></div>
          </div>
          <!-- 待确认 -->
          <div v-for="reorder in reorders" :key="reorder.id" ><div v-if="reorder.status=='0'">
            <div class="white">
            <p class="state">待确定</p>
            <p class="name">{{reorder.goods.goodsName}}</p>
            <p class="time">{{reorder.createTime}}</p>
            <button @click="ok(reorder.billId)" class="btn3">确认</button>
              </div></div>
          </div>
          <!-- 已完成 -->
          <div v-for="reorder in reorders" :key="reorder.id" ><div v-if="reorder.status=='1'">
            <div class="white">
            <p class="state">已完成</p>
            <p class="name">{{reorder.goods.goodsName}}</p>
            <p class="time">{{reorder.createTime}}</p>
  ...
  data(){
    ...
    return Promise.reject(error);
            ...
            //   查询当前用户所提交订单
              ...
              axios.post('http://5101g0918f.wicp.vip/bill/showOthersBills',data2,
                )
              .then((res)=>{
                this.reorders=res.data.data;
                  console.log("111",res); 
                  if(res.data.code==401)alert("请登录");
              })
              .catch((error)=>{
                  console.log(error)  
              });
